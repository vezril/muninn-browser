...
## 3c. externally_connectable handshake fixes — "missing permissions" CLEARED (gates 5–6, 2026-07-18)

- [x] 3c.1 Expose ONLY `window.chrome` (drop `window.browser`) in `externally-connectable.js` — Proton's Mozilla polyfill wraps `chrome` only when `window.browser` is undefined; our minimal `window.browser` broke it (`t4.runtime` undefined). Fixed → the account app began sending `pass-installed`.
- [x] 3c.2 `fireMessage` (worker) now supports **Promise-returning** onMessage listeners (Proton's `lU.onMessage` is async) — the response was previously dropped → we sent null. Test: `testExternalMessagePromiseReturningListener` (green).
- [x] 3c.3 Programmatic gate signal (`MessageBroker.onExternalRelay`): logs the external message `type` (safe discriminator) + round-trip completion — replaces flaky manual console probing. Gate 6: `pass-installed → RESPONDED`, UI advanced to "Welcome to your new password manager." **Detection/permission barrier RETIRED.**

## 4. The human gate (auth-fork-login — the Risk-1 go/no-go) — DETECTION CLEARED; now at the session-fork step

- [x] 4.1 **[HUMAN GATE — ground rule 2]** Warned Calvin; launched the app (visible window) on his confirmation — twice.
- [~] 4.2 **[HUMAN GATE — ground rule 1]** Calvin at the keyboard, no credentials touched. **RESOLVED across 6 gates:** fork trigger + onboarding load ✓; externally_connectable bridge installed live ✓; **"missing permissions" CLEARED** (gates 5–6 — drop `window.browser` + Promise-listener fix) → onboarding advanced to "Welcome to your new password manager." Sign-in then sent the `fork` message (delivered + RESPONDED), but the UI showed **"Unable to Sign In to Proton Pass. Unknown error."** → now blocked at the **session-fork completion** (deeper than detection). Leading cause: **Decision 4 cookie/store topology** (dedicated host `WKWebsiteDataStore` isolates session/cookies from the account tab). See `research/e6-external-gate-2026-07-17.md` (RESOLUTION + NEXT BLOCKER).
