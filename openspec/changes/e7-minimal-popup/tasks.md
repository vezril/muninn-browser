...
## PARKED (2026-07-21) — auth-fork consume still "Invalid fork state"

Across fork-init (/authorize) and onboarding paths + storing f<state> from the
onboarding URL AND the /authorize navigation (URL KVO): the `fork` message fires and
RESPONDS, but there is STILL no `pullFork` fetch — consume throws "Invalid fork state"
before any network. So the `state` we store does not match the fork message's
`payload.state`, OR `localState`={} is insufficient (consume may need real localState
content, or the fork uses a state generated by a context we don't intercept —
fork.js/account app, not a tab navigation).

**Resume plan:** compare the ACTUAL states — instrument (ground-rule-1-safe: log only
whether `getItem("f"+payload.state)` HIT/MISSED in the consume, not the values) to see
if it's a key mismatch vs a localState-content problem. Options: (1) intercept where the
extension REALLY stores f<state> (fork.js on account.proton.me? a message from the
account app to the extension?) and mirror it; (2) get the popup to render (deep E7) so
Proton's own fork-init runs with correct state+localState. Parked at Calvin's direction
to build usable features toward the daily-driver goal first.
