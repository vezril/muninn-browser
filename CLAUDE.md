...
**E5 (general injection — FR-9) is BUILT + green (31 tests; change `e5-injection-frame-registry`).** Unified isolated-world Proxy polyfill (`content-polyfill.js`) with a content-world transport; `ForkBridgeInjector` injects the full manifest set — bootstrap → content-polyfill (isolated, `document_start`) → `orchestrator.js` (isolated, `document_end`, all frames) → `webauthn.js` (MAIN, `document_start`); `fork.js` nav-gated to account.proton.me. S2 MAIN-world isolation holds under the full surface. `FrameRegistry` (main id 0, stable subframe ids) backs `webNavigation.getAllFrames`/`getFrame`; `runtime.getFrameId` resolves the caller's frame via `message.frameInfo` (Safari current-frame form; element-arg autofill is post-MVP). Ports **deferred** (Decision 4b — orchestrator initiates none). Orchestrator boot audit: `research/orchestrator-audit-2026-07-17.md` (headless boot is offscreen-limited; the live gate is authoritative).
